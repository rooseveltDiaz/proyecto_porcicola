...
class empleadoTableClass extends empleadoBaseTableClass {
    ...
    public static function validateCreate($nombre_completo, $direccion, $numero_documento, $telefono) {
        ...
        else if(strlen($numero_documento)<5){
          session::getInstance()->setError('Minimo 5 caracteres en el numerp documento');
            $flag = true;
            session::getInstance()->setFlash(empleadoTableClass::getNameField(empleadoTableClass::NUMERO_DOC, true), true);
     ...
     public static function isUnique($numero_documento) {
      $numero_documento = '\\' . camelCase::getInstance()->camelCase(empleadoTableClass::NUMERO_DOC) . "TableClass";
     }
